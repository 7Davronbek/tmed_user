...
import {
  $serviceList,
  fetchServiceInfinityFx,
  getServiceInfinityListEv,
} from "@/entities/clinic-services";
...
export const ClinicServiceCard = () => {
  const [{ results, next, next_offset }, isLoading] = useUnit([
    $serviceList,
    fetchServiceInfinityFx.pending,
  ]);
  return (
    <div>
      {isLoading ? (
        ...
        <InfiniteScroll
          ...
          next={() =>
            getServiceInfinityListEv({ limit: 12, offset: next_offset })
          }
        ...
        >
          <div className={sx.wrap}>
            {results &&
              results.map((item) => {
                return <ClinicServiceCardDetail key={item.id} service={item} />;
              })}
          </div>
        </InfiniteScroll>
      )}
